// Core generation flow, shared by the web server actions (Google-auth'd
// users) and the embedded admin app (session-token-auth'd shops). Callers
// resolve `userId` themselves; everything here is auth-agnostic.
...
// What generation UIs render from — a snapshot of the spin's state.
// Terminal states carry the result; "generating" carries startedAtMs so a
// reopened tab resumes the progress bar at the right spot.
...
// Generations cost real money (~$0.71/run) so we NEVER regenerate unless
// the owner passes { force: true }. Results persist on the row — embeds and
// re-visits are free forever.
//
// Async path: submit to fal's queue and return immediately with status
// "generating". Completion happens via the fal webhook and/or status
// polling — see lib/spin-completion.ts.
...
// one is in flight. This is the poll target — and the ONLY finish path on
// localhost, where fal's webhook can't reach us.
export async function getGenerationStatus(userId: string, spinId: string): Promise<SpinStatusPayload> {
  const provider = getSpinVideoProvider();
  ...
  // a crashed run or a pre-async legacy row. Nothing will ever finish
  ...
  if (spin.status === "generating" && spin.falRequestId) {
    try {
      ...
      console.error("[generation] reconcile failed, will retry:", err);
    }
  }
